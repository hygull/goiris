...
func SendFeedback(ctx *iris.Context) {
	...
	uid := strings.TrimSpace(ctx.FormValueString("uid"))
	...
	rows, id := ExecuteQuery(ctx, db, query, 1) //2nd parameter denotes the status_code-100...eg. for 101 we have to give 1
	if rows == nil && id == 0 {
		return
	}
	if rows != nil {
